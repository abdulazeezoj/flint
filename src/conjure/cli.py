"""The `conjure` command-line app.

See PRODUCT_FLOW.md §1 for the entry-point table this implements: bare
`conjure` behaves like `conjure new` (an interactive wizard), `conjure new
...
from conjure import generator, postgen, prefs, prompts
...
def root(
    ...
    if version:
        console.print(f"conjure {__version__}")
        raise typer.Exit()
...
def new(
    ...
    remember: Annotated[
        ...
        typer.Option(
            ...
            help=(
                "Remember these choices in ~/.conjure/last.json as the "
...
def list_templates_cmd() -> None:
    ...
    console.print(table)
    console.print(
        "\nPass a pair with e.g. [bold]conjure new my-api --framework fastapi "

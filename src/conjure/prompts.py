...
from conjure.generator import FrameworkMeta, TemplateMeta, when_matches
...
def _select_enabled(
    ...
    if value is not None:
        ...
        if not by_id[value].enabled:
            raise ConjureUserError(f"'{value}' isn't available yet.")
        return value
...
def parse_option_flags(raw_options: list[str]) -> dict[str, str]:
    ...
    for item in raw_options:
        if "=" not in item:
            raise ConjureUserError(f"--option '{item}' must be in key=value form.")
...
def _parse_bool_flag(key: str, raw: str) -> bool:
    ...
    if normalized in _FALSY:
        return False
    raise ConjureUserError(f"'{raw}' is not a valid boolean for --option {key} (use true/false).")

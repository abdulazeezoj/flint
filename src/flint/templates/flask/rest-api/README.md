...
- **Layout**: `routes/` (not `blueprints/`) for cross-framework
  consistency — a developer moving between a flint-generated FastAPI
  project and a flint-generated Flask project should find the same
...
`src/flint/postgen.py`'s `print_summary` hardcodes the FastAPI-specific
...
live via `flint new ... --framework flask --template rest-api`. Fixing

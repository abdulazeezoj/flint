...
- **Layout**: `routes/` (not `blueprints/`) for cross-framework
  consistency — a developer moving between a brupy-generated FastAPI
  project and a brupy-generated Flask project should find the same
...
`src/brupy/postgen.py`'s `print_summary` hardcodes the FastAPI-specific
...
live via `brupy new ... --framework flask --template rest-api`. Fixing

...
from brupy.generator import FrameworkMeta, TemplateMeta, when_matches
...
def prompt_project_name(name: str | None, interactive: bool) -> tuple[str, str, str]:
    ...
    while True:
        ...
        if answer is None:
            raise BrupyUserError("Cancelled.")
        try:
...
def _select_enabled(
    ...
    if value is not None:
        ...
        if not by_id[value].enabled:
            raise BrupyUserError(f"'{value}' isn't available yet.")
        return value
...
def parse_option_flags(raw_options: list[str]) -> dict[str, str]:
    ...
    for item in raw_options:
        if "=" not in item:
            raise BrupyUserError(f"--option '{item}' must be in key=value form.")
...
def _parse_bool_flag(key: str, raw: str) -> bool:
    ...
    if normalized in _FALSY:
        return False
    raise BrupyUserError(f"'{raw}' is not a valid boolean for --option {key} (use true/false).")

...
- **skills** — references (by `id`, each with its own `when`) into a
  *shared* catalog at `src/brupy/skills/<id>/`, decoupled from any one
...
def get_skill(skill_id: str) -> SkillMeta:
    ...
    Raises `BrupyError` (not `BrupyUserError`) if `skill_id` has no
    ...
    if not meta_path.is_file():
        raise BrupyError(f"Unknown skill '{skill_id}' referenced by a template.")
...
def get_framework(framework_id: str) -> FrameworkMeta:
    ...
    meta_path = framework_dir / "template.json"
    if not meta_path.is_file():
        raise BrupyUserError(f"Unknown framework '{framework_id}'.")
    data = _read_json(meta_path)
...
def get_template(framework_id: str, template_id: str) -> TemplateMeta:
    for template in list_templates(framework_id):
        if template.id == template_id:
            return template
    raise BrupyUserError(f"Unknown template '{template_id}' for framework '{framework_id}'.")
...
def render(
    ...
    ``BrupyUserError`` if either the framework/template is disabled or the
    ...
    if not framework.enabled:
        raise BrupyUserError(f"Framework '{framework_id}' is not available yet.")

    template = get_template(framework_id, template_id)
    if not template.enabled:
        raise BrupyUserError(f"Template '{template.full_id}' is not available yet.")
    ...
    try:
        ...
        if isinstance(exc, BrupyError):
            raise
        raise BrupyError(f"Failed to generate project: {exc}") from exc
...
def _write_skills_index(target_dir: Path, skills: list[SkillMeta]) -> Path:
    ...
    lines = [
        ...
        "this project uses — generated by brupy to match the choices made",

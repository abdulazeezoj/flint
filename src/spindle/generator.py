...
- **skills** — references (by `id`, each with its own `when`) into a
  *shared* catalog at `src/spindle/skills/<id>/`, decoupled from any one
...
def get_skill(skill_id: str) -> SkillMeta:
    ...
    Raises `SpindleError` (not `SpindleUserError`) if `skill_id` has no
    ...
    if not meta_path.is_file():
        raise SpindleError(f"Unknown skill '{skill_id}' referenced by a template.")
...
def get_framework(framework_id: str) -> FrameworkMeta:
    ...
    meta_path = framework_dir / "template.json"
    if not meta_path.is_file():
        raise SpindleUserError(f"Unknown framework '{framework_id}'.")
    data = _read_json(meta_path)
...
def get_template(framework_id: str, template_id: str) -> TemplateMeta:
    for template in list_templates(framework_id):
        if template.id == template_id:
            return template
    raise SpindleUserError(f"Unknown template '{template_id}' for framework '{framework_id}'.")
...
def render(
    ...
    ``SpindleUserError`` if either the framework/template is disabled or the
    ...
    if not framework.enabled:
        raise SpindleUserError(f"Framework '{framework_id}' is not available yet.")

    template = get_template(framework_id, template_id)
    if not template.enabled:
        raise SpindleUserError(f"Template '{template.full_id}' is not available yet.")

    if target_dir.exists() and any(target_dir.iterdir()) and not force:
        raise SpindleUserError(
    ...
    try:
        ...
        if isinstance(exc, SpindleError):
            raise
        raise SpindleError(f"Failed to generate project: {exc}") from exc
...
def _write_skills_index(target_dir: Path, skills: list[SkillMeta]) -> Path:
    ...
    lines = [
        ...
        "this project uses — generated by spindle to match the choices made",

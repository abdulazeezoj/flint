...
- **Layout**: `routes/` (not `blueprints/`) for cross-framework
  consistency — a developer moving between a spindle-generated FastAPI
  project and a spindle-generated Flask project should find the same
...
`src/spindle/postgen.py`'s `print_summary` hardcodes the FastAPI-specific
...
live via `spindle new ... --framework flask --template rest-api`. Fixing

"""The `spindle` command-line app.

See PRODUCT_FLOW.md §1 for the entry-point table this implements: bare
`spindle` behaves like `spindle new` (an interactive wizard), `spindle new
...
from spindle import generator, postgen, prefs, prompts
...
def root(
    ...
    """Spindle — interactive project scaffolding for Python frameworks."""
    if version:
        console.print(f"spindle {__version__}")
        raise typer.Exit()
...
def new(
    ...
    remember: Annotated[
        ...
        typer.Option(
            ...
            help=(
                "Remember these choices in ~/.spindle/last.json as the "
...
def list_templates_cmd() -> None:
    ...
    console.print(table)
    console.print(
        "\nPass a pair with e.g. [bold]spindle new my-api --framework fastapi "
...
def _run_new(
    *,
    ...
    except SpindleUserError as exc:

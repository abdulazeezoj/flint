...
```bash
brupy new my-api --framework flask --template hello-world --yes
```
...
Brupy's two frameworks isn't a relearn. Reach for
...
!!! note
    ...
    — Brupy uses `main.py` for every framework's entrypoint so the landmark
...
```bash
brupy new my-api --framework flask --template hello-world --docker --yes
```
...
Brupy — one `Dockerfile` is the entire Docker story.

...
Every project Conjure generates comes from exactly one `<framework>/<template>`
...
- **Option** — a further, template-specific choice, declared by the template
  itself — not hardcoded in Conjure. `hello-world` asks only whether to add
  ...
  Redis. Conjure's wizard has no built-in notion of "database" or "worker" —
  it just renders whatever the chosen template's `template.json` declares.

```text
conjure new my-api --framework fastapi --template rest-api -o database=postgres
#                 ^^^^^^^^^^^^^^^^^^^ ^^^^^^^^^^^^^^^^^^^ ^^^^^^^^^^^^^^^^^^^
#                 framework           template             template option
```

This is what lets Conjure grow in three independent directions — more
...
You don't need to know this to use Conjure, but it explains why the pages
...
!!! tip
    ...
    `conjure list-templates` — it prints every framework/template pair,
    including anything still "coming soon," with no project generated. See
    [CLI Reference](../cli-reference.md).

## Next

- [Getting Started](../getting-started.md) — install Conjure and generate
  your first project

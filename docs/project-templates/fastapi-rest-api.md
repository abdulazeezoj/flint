...
The richest template Conjure ships. Where `hello-world` gets you a single
...
- **Strictly opinionated** — Conjure always uses this exact name and
...
- **A default, not a rule** — `schemas.py` and `models.py` stay
  ...
  folders the moment your project grows a second resource — Conjure just
...
equivalent if you just run `conjure new my-api` and answer the prompts
instead.
...
Pass `--docker` and Conjure adds a `Dockerfile` (plus a matching

...
brupy-generated FastAPI project and a brupy-generated Flask project should
...
Redis as your broker shouldn't make Brupy ask twice. The rule: `redis`
...
Pass `--docker` and Brupy adds a `Dockerfile` (`uv`-based, matching
every other brupy template) plus `.dockerignore`. The one

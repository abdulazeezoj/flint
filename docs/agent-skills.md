...
in from a bug brupy's own templates hit while being built:
...
`taskiq`, since that worker is FastAPI/async-only in Brupy today.
...
Instead, the catalog lives once, flat, at `src/brupy/skills/<id>/` —
...
After rendering every matched skill, Brupy also writes
...
"fix" that reintroduces a bug brupy's own templates already hit once. The
...
speculative either — they're bugs brupy's own templates hit while being

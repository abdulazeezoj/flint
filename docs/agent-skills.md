...
in from a bug spindle's own templates hit while being built:
...
`taskiq`, since that worker is FastAPI/async-only in Spindle today.
...
Instead, the catalog lives once, flat, at `src/spindle/skills/<id>/` —
...
After rendering every matched skill, Spindle also writes
...
"fix" that reintroduces a bug spindle's own templates already hit once. The
...
speculative either — they're bugs spindle's own templates hit while being

...
migrations yet, whether background work needs a queue. Brupy answers the
...
- **Zero to running in under a minute.** No install, no config file — run
  `uvx brupy` and you have a project you can `uv run` immediately.
...
- **Fully scriptable.** Every interactive prompt has a matching flag, so
  `brupy new` works identically in CI with zero prompts.
...
- **[Getting Started](getting-started.md)**
  Install Brupy and generate your first project, interactively or
  non-interactively.
...
```bash
uv tool install brupy   # persistent `brupy` on PATH
# or run it ephemerally, no install:
uvx brupy
```

Brupy is a CLI, not a library: installing it puts a `brupy` command on
your `PATH`. The PyPI distribution and the command share the same
name, so `uvx brupy` works with no `--from` needed.

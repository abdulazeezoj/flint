...
**Status:** Draft for v0
**Owner:** Engineering
**Last updated:** 2026-08-13 (v0.13: project renamed to Spindle, second time — see PRODUCT_SPEC.md §11; docs-site grid-cards gotcha)

Implements `PRODUCT_SPEC.md` / `PRODUCT_FLOW.md`. This is the technical
design for the `spindle` CLI itself (not the projects it generates).
...
| Templating | [Jinja2](https://jinja.palletsprojects.com/) | Renders both file contents and file/directory *names* (`{{package_name}}`), industry-standard, no need for a full scaffolding framework (cookiecutter/copier) when Spindle owns its own bundled templates. |
| Packaging / env | [uv](https://docs.astral.sh/uv/) | Both: (a) how Spindle itself is built/packaged/tested, and (b) the package manager wired into every generated project. |
...
- PyPI distribution name: `spindle` (see PRODUCT_SPEC §10 for why).
- Console script entry point: `spindle`.
- Primary usage is via `uvx spindle` (ephemeral run, no install) or
  `uv tool install spindle` (persistent `spindle` on PATH) — mirrors how
...
│       ├── prefs.py             # ~/.spindle/last.json — best-effort read/write
│       ├── errors.py            # SpindleError and friends -> exit codes
│       └── templates/
│           ├── fastapi/
...
5. On any exception: remove everything written so far under `target_dir`
   ...
   delete something Spindle didn't create. All-or-nothing generation, per
   PRODUCT_FLOW §5.
...
- **Inline `{% if %}`** (acceptable for "gathering" files): some files
  ...
  "merge multiple layers' contributions into one file" mechanism Spindle
...
Everything above is about how spindle's *own* code decides what to
...
- **Strictly opinionated** (spindle always uses this exact name/location,
...
- **A default, not a rule** — spindle picks *something* reasonable so the
  ...
  second resource, but spindle doesn't make that call for the user.
...
`tasks/` structure — a developer who's used one spindle template shouldn't
...
`src/spindle/skills/<id>/` — outside `templates/` entirely — and each
...
**Catalog entry shape**, `src/spindle/skills/<id>/`:
...
- **This document set** (`docs/_product/PRODUCT_SPEC.md`/`PRODUCT_FLOW.md`/
  `PRODUCT_ARCH.md`) is for whoever is *building* spindle — spec-numbered
...
- **The docs site** (`docs/*.md` outside `_product/`, built by
  ...
  for whoever is *using* spindle — install instructions, a CLI reference,
...
`~/.spindle/last.json` (PRODUCT_FLOW.md §6) is deliberately the simplest
thing that could work: one flat JSON file, no schema/migration
machinery, best-effort I/O.

```python
PREFS_DIR = Path.home() / ".spindle"
...
- `load_prefs() -> dict` / `save_prefs(prefs: dict) -> None` — the only
  ...
  file must never be the reason `spindle new` fails.
...
This means a template's schema can change across Spindle versions — an
...
|---|---|
| Entry points, flag parsing | `cli.py` (Typer app; `new` command; bare `spindle` invokes `new` via Typer's default-command pattern) |
...
| Exit codes | `errors.py` defines `SpindleUserError` (→1) and lets anything else bubble to Typer's default handler (→2); the top-level command wraps generation in a `try/except SpindleUserError` |
...
`spindle list-templates` (since v0.10) is a separate, standalone `@app.
...
`pytest` runs with `--cov=spindle --cov-report=term-missing
...
line *or* branch in `src/spindle/` goes uncovered, not just when a whole
...
- `test_generator.py` — the bulk of the suite. Covers: `when_matches`
  ...
  now), rollback-on-failure (including that a `SpindleError` mid-render
...
- `test_prefs.py` — `load_prefs`/`save_prefs` round-tripping, a missing/
  ...
  touching the real `~/.spindle`, and from leaking remembered state between
...
- `test_main.py` — covers the `python -m spindle` / `python -m spindle.cli`
  entry-point guards via `runpy.run_module(..., run_name="__main__")`,
  plus a plain `import spindle.__main__` to cover the guard's not-taken
  branch.
...
- `src/spindle/__init__.py` holds `__version__`, single source of truth;
...
- **CI** (`.github/workflows/ci.yml`, since v0.10.1): on every push to
  ...
  script actually runs (`uv run spindle --version`). Single Python
  version (3.11, the floor in `requires-python`) — spindle's own code has
...
`--force` overwrite confirmation and `spindle list-templates` shipped;

# Spindle — Product Flow

**Status:** Draft for v0
**Last updated:** 2026-08-13 (v0.10: interactive `--force` confirmation, `spindle list-templates`)

Companion to `PRODUCT_SPEC.md`. Describes exactly what happens when a user
runs Spindle, in both interactive and non-interactive modes. See
...
| Command | Behavior |
|---|---|
| `uvx spindle` / `spindle` | No args → full interactive wizard, generates into a new directory named after the answered project name, in the current working directory. |
| `spindle new [NAME]` | Same wizard; `NAME` pre-fills the project-name prompt (or skips it if `--yes`). |
| `spindle new NAME --framework fastapi --template rest-api -o database=postgres -o orm=sqlmodel --docker --git --install --yes` | Fully non-interactive; no prompts, generates immediately. `--option`/`-o key=value` is repeatable, one per template-declared option. |
| `spindle new NAME --no-remember ...` | Same as above, but neither reads nor writes `~/.spindle/last.json` for this run (§6). |
| `spindle list-templates` | Prints a table of every framework/template pair (label, description, `--docker` support), including disabled/"coming soon" entries. Generates nothing — pure introspection. |
| `spindle --version` | Prints version, exits. |
| `spindle --help` / `spindle new --help` | Prints usage, exits. |

`spindle` with no subcommand is an alias for `spindle new` (matches the
...
hardcoded default — if `~/.spindle/last.json` remembers a value for that
...
5. Template options — declared by the chosen template, asked in the
   order it declares them; entirely different per template (Spindle's own
...
7. Add a Dockerfile?
   ...
   - Default **no** (unlike git/install below, this one opts in rather
     ...
     yet, Spindle warns and continues without one rather than failing.
...
Triggered by either passing `--yes`, or by Spindle detecting stdin is not a
...
- If a required decision has no safe default and no flag was given (there
  are none in v0 — every prompt has a default), Spindle would exit `1` with
...
## 6. Remembered preferences (`~/.spindle/last.json`)

After a **successful** generation (files written; git/install steps can
still fail independently without affecting this), Spindle records:
...
This is stored in `~/.spindle/last.json`. On the *next* run:
...
- A remembered value that's no longer valid (e.g. a select option whose
  choices changed, or a stale key from an older Spindle version) is
...
`--no-remember`, Spindle neither reads nor writes `~/.spindle/last.json` for
...
(e.g. a read-only home directory), Spindle silently proceeds as if nothing
were remembered — this never fails or warns during generation. There is
no version/schema field; an entry from an older Spindle that no longer
...
```
$ spindle new my-api --framework fastapi --template hello-world --git --install --yes

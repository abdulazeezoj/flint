...
Spindle remembers what you picked last time and uses it as the new default —
...
After a **successful** generation, Spindle saves three things to
`~/.spindle/last.json`:
...
error) never touches `~/.spindle/last.json`; whatever was remembered before
...
say you upgraded Spindle and a `select` option's choices changed, or an
option was renamed or removed entirely — Spindle doesn't error and doesn't
...
Reading and writing `~/.spindle/last.json` is entirely best-effort. If the
file is missing, unreadable, not valid JSON, or isn't even a JSON object
(e.g. hand-edited into something odd), Spindle treats it as empty and
...
`~/.spindle/last.json` for that one invocation — Spindle behaves as if the
...
`~/.spindle/last.json` is a plain JSON file — nothing sensitive, nothing
...
!!! tip "Resetting to defaults"
    ...
    safe, complete reset: `rm ~/.spindle/last.json`. Spindle will recreate it
...
That run succeeds, so Spindle remembers `fastapi`, `rest-api`, `database=postgres`,
and `docker=true` (along with whatever defaults applied to the rest of the
`rest-api` options and to git/install). Run Spindle again with no arguments:

...
def test_render_propagates_conjure_error_without_wrapping_but_still_rolls_back(
    tmp_path: Path, monkeypatch
):
    # A ConjureError raised mid-render (e.g. surfaced from a future layer
    ...
    def raise_user_error(layer_root, target_dir, context):
        raise ConjureUserError("deliberate")

    monkeypatch.setattr(generator_module, "_render_layer", raise_user_error)

    with pytest.raises(ConjureUserError, match="deliberate"):
        render("fastapi", "hello-world", target, make_answers())
...
def test_render_does_not_delete_preexisting_directory_on_failure(tmp_path: Path, monkeypatch):
    ...
    # directories conjure itself created get rolled back.
    target = tmp_path / "my-api"
    target.mkdir()
...
class TestSkillsMechanism:
    ...
    def test_get_skill_unknown_raises_conjure_error(self, tmp_path: Path, monkeypatch):
        monkeypatch.setattr(generator_module, "SKILLS_DIR", tmp_path)

        with pytest.raises(ConjureError, match="Unknown skill 'ghost'"):
            get_skill("ghost")

...
from brupy import __version__, cli, generator, postgen, prefs
from brupy.cli import app
...
def test_help():
    ...
    assert result.exit_code == 0
    assert "brupy" in result.stdout.lower()
...
def test_bare_invocation_defaults_to_new(tmp_path: Path, monkeypatch):
    # Bare `brupy` defaults git-init and install to True (FR1); stub both
...
def test_list_templates_cmd_lists_real_frameworks_and_templates():
    ...
    assert "brupy new my-api" in result.stdout

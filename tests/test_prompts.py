...
from conjure import prompts
from conjure.errors import ConjureUserError
from conjure.generator import FrameworkMeta, TemplateLayer, TemplateMeta, TemplateOption
...
def test_prompt_template_options_unknown_key_rejected():
    with pytest.raises(ConjureUserError, match="Unknown option"):
...
def test_prompt_template_options_provided_invalid_select_value_rejected():
    with pytest.raises(ConjureUserError, match="not a valid value"):
...
def test_prompt_template_options_provided_invalid_confirm_value_rejected():
    with pytest.raises(ConjureUserError, match="not a valid boolean"):
        prompts.prompt_template_options(
            WIDGET_TEMPLATE,
...
def test_prompt_template_options_real_rest_api_template_default_flow():
    from conjure.generator import get_template

"""Covers the `python -m brupy` / `python -m brupy.cli` entry-point guards.
...
def test_dunder_main_invokes_cli(monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", ["brupy", "--version"])
    with pytest.raises(SystemExit) as exc_info:
        runpy.run_module("brupy", run_name="__main__")

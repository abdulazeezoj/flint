...
migrations, or a background worker — flint's `rest-api` template wires
all of that from the start, which is real effort saved over building it
by hand.

## Quick start

Flint's PyPI package name (`flint-kit`) differs from the command
(`flint`) — plain `flint` was already taken on PyPI by an unrelated
package. This matters for exactly one thing: **ephemeral runs need
`--from`**, or `uvx` will try to resolve the wrong package.

```bash
# Run once, without installing (note --from — bare `uvx flint` resolves incorrectly):
uvx --from flint-kit flint new my-api --framework fastapi --template hello-world --yes

# Or install the `flint` command permanently on PATH:
...
`uv`-managed, and flint uses it for the optional dependency-install
step.
...
```bash
uvx --from flint-kit flint new my-api \
...
flint itself before any project exists.

## Gotchas

- **`uvx flint` alone will not work.** The PyPI package is `flint-kit`;
  the command is `flint`. Always use `uvx --from flint-kit flint ...`
  for ephemeral runs. `uv tool install flint-kit` doesn't need this —
  it installs the `flint` console script under its own name regardless
...
- **Remembered preferences can silently change defaults.** After a
  successful run, flint remembers the framework/template/options
  chosen and reuses them as defaults next time (`~/.flint/last.json`).

...
Every `brupy` command, flag, and exit code.
...
from `~/.brupy/last.json`, if one exists and is still valid. An
...
Two things trigger non-interactive mode: passing `--yes`/`-y`, or brupy
...
input. Brupy works down a fallback chain: the flag or `-o` value if you
...
**non-empty**, brupy refuses to touch it unless `--force` is passed —
...
worker, broker, and so on) — brupy's core has no built-in knowledge of
...
After a successful generation, brupy saves the framework, template,
and every resolved option (plus `--docker`/`--git`/`--install`) to
`~/.brupy/last.json`, and uses them as the new default next time. An
...
remembering/reading `~/.brupy/last.json`:
